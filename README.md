...
4. **Render でデプロイする場合**  
   ...
   - **LINE**: Webhook URL は次のどちらか（末尾スラッシュなし推奨）  
     - `https://（サービスURL）/line/webhook`  
     - `https://（サービスURL）/webhook`（短いパス。404 回避用）  
...
| POST | `/line/webhook` / `/webhook` | LINE Messaging API（同一処理。短い URL は後者） |
